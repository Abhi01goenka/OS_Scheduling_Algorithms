...
void input(){
    ...
    for(int i=1; i<=n; ++i){
        cin>>v[i].pid>>v[i].priority>>v[i].arrival_time>>v[i].burst_time;
    }
    cout<<"Enter highest priority : ";
    cin>>highest_priority;
    for(int i=1; i<=n; ++i){
        ma=max(ma,v[i].priority);
    }
}

void output(){
    cout<<"\n---------------------------------------------------------------------------------------------------"<<endl;
    cout<<"\nPID\tP\tAT\tBT\tCT\tTAT\tWT\tRT\n";
    for(int i=1; i<=n; ++i)cout<<v[i].pid<<"\t"<<v[i].priority<<"\t"<<v[i].arrival_time<<"\t"<<v[i].burst_time<<"\t"<<v[i].completion_time<<"\t"<<v[i].turnaround_time<<"\t"<<v[i].waiting_time<<"\t"<<v[i].response_time<<endl;
    cout<<"\n--------------------------------------------------------------------------------------------------"<<endl;
}
