...
void HRRN(){
    ...
    while(i<=n){
        ...
        if(ms.empty()){
            ...
            while(!ms.empty() && i<=n){
                multiset<pair<double, ll>> ms1=ms;
                ms.clear();
                for(auto ele:ms1){
                    double ratio=((double)(curr_time-v[ele.second].arrival_time+v[ele.second].burst_time))/((double)v[ele.second].burst_time);
                    ms.insert({ratio, ele.second});
                }
                auto it=ms.end();
                it--;
...
int main(){
    cout<<"Highest Response Ratio Next (HRRN)"<<"\n\n";
    input();
    HRRN();
    output();
}